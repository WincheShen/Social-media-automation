...
每个小红书账号需要登录一次，之后 cookie 自动持久化。

> **不需要手动启动 Chrome。** `cli.py` 的每个命令都会自动检测并启动 Chrome 实例（通过 `ensure_chrome()`），每个端口对应一个独立的 Chrome Profile，登录态互相隔离。

```bash
cd vendor/xiaohongshu-skills

# 登录 XHS_01（自动启动 Chrome，端口 9222）
python3 scripts/cli.py --port 9222 login

# 登录 XHS_02（自动启动 Chrome，端口 9223）
python3 scripts/cli.py --port 9223 login

# 登录 XHS_03（自动启动 Chrome，端口 9224）
python3 scripts/cli.py --port 9224 login
```

执行后会弹出 Chrome 窗口并打开小红书登录页，用对应手机上的小红书 App 扫码即可。

**验证登录是否成功：**

```bash
python3 scripts/cli.py --port 9222 check-login
# ✅ 成功输出: {"logged_in": true, "nickname": "学霸学长", ...}
# ❌ 失败输出: exit code 1
```
...
| `XhsTimeoutError` | CLI 命令会自动启动 Chrome，如仍超时检查 Chrome 是否安装 |
